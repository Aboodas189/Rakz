...
# Initialize global variables for phone detection
last_phone_detection_time = None
phone_detected_in_last_frame = False
phone_boxes = []
last_focus_decrease_time_due_to_phone = None
...
def process_frame(frame, face_mesh, yolo_model, focus_score, last_look_centered_time, not_looking_start_time, blink_start_time, blink_detected, eyes_closed_start_time, last_focus_increase_time, last_focus_decrease_time, last_phone_detection_time, phone_detected_in_last_frame, phone_boxes, last_focus_decrease_time_due_to_phone):
    ...
    current_time = time.time()  # Ensure it's available for all branches

    # Phone detection logic (process 1 frame per second)
    if last_phone_detection_time is None or current_time - last_phone_detection_time >= 1:
        ...
        for r in yolo_results:
            ...
            for box in boxes:
                cls = int(box.cls[0])
                conf = box.conf[0]
                if cls == 67 and conf > 0.8:  # COCO class ID for 'cell phone' is 67
                    phone_detected_in_last_frame = True
                    x1, y1, x2, y2 = map(int, box.xyxy[0])
                    phone_boxes.append((x1, y1, x2, y2, conf))
        last_phone_detection_time = current_time

    # Decrease focus score if phone is detected
    if phone_detected_in_last_frame:
        if last_focus_decrease_time_due_to_phone is None or current_time - last_focus_decrease_time_due_to_phone >= 1:
            focus_score = max(0, focus_score - 10)
            last_focus_decrease_time_due_to_phone = current_time

    # Overlay phone detection results
    for (x1, y1, x2, y2, conf) in phone_boxes:
        cv.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 255), 2)
        cv.putText(frame, f'Phone: {conf:.2f}', (x1, y1 - 10), FONTS, 0.7, (0, 255, 255), 2)
    ...
    return (frame, focus_score, last_look_centered_time, not_looking_start_time, blink_start_time, blink_detected, eye_direction_text, face_position, last_focus_increase_time, last_focus_decrease_time, last_phone_detection_time, phone_detected_in_last_frame, phone_boxes, last_focus_decrease_time_due_to_phone)
...
def video_frame_callback(frame: av.VideoFrame) -> av.VideoFrame:
    ...
    global last_phone_detection_time, phone_detected_in_last_frame, phone_boxes, last_focus_decrease_time_due_to_phone
    global yolo_model
    ...
    with mp_face_mesh.FaceMesh(
        ...
        (img, focus_score, last_look_centered_time, not_looking_start_time, 
         blink_start_time, blink_detected, _, _, last_focus_increase_time, last_focus_decrease_time,
         last_phone_detection_time, phone_detected_in_last_frame, phone_boxes, last_focus_decrease_time_due_to_phone) = process_frame(
            img, face_mesh, yolo_model, focus_score, last_look_centered_time, not_looking_start_time, 
            blink_start_time, blink_detected, eyes_closed_start_time, last_focus_increase_time, last_focus_decrease_time,
            last_phone_detection_time, phone_detected_in_last_frame, phone_boxes, last_focus_decrease_time_due_to_phone
        )
...
def process_uploaded_video(video_file):
    ...
    # Initialize variables for phone detection
    last_phone_detection_time = None
    phone_detected_in_last_frame = False
    phone_boxes = []
    last_focus_decrease_time_due_to_phone = None
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while cap.isOpened():
            ...
            (frame, focus_score, last_look_centered_time, not_looking_start_time, 
             blink_start_time, blink_detected, eye_direction, face_position, last_focus_increase_time, last_focus_decrease_time,
             last_phone_detection_time, phone_detected_in_last_frame, phone_boxes, last_focus_decrease_time_due_to_phone) = process_frame(
                frame, face_mesh, yolo_model, focus_score, last_look_centered_time, not_looking_start_time, 
                blink_start_time, blink_detected, eyes_closed_start_time, last_focus_increase_time, last_focus_decrease_time,
                last_phone_detection_time, phone_detected_in_last_frame, phone_boxes, last_focus_decrease_time_due_to_phone
            )
